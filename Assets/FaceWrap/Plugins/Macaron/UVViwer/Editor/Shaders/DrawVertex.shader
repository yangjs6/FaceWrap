Shader "Hidden/Macaron/UVViewer/Editor/DrawVertex"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            {
                float constScale = 1.0f / 2048;
                float2 offset = (mul(unity_ObjectToWorld, float4(-1, 1, 0, 0)).xy) * constScale;
                ...
                offset = (mul(unity_ObjectToWorld, float4(1, 1, 0, 0)).xy) * constScale;
                worldPos = p[0].vertex;
                worldPos.xy += offset * _Radius;
