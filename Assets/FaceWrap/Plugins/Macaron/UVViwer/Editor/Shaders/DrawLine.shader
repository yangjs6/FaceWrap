Shader "Hidden/Macaron/UVViewer/Editor/DrawLine"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            {
                float constScale = (mul(unity_ObjectToWorld, float4(1, 1, 0, 0)).xy).x * 1.0f / 2048 * 0.5;
                constScale = clamp(constScale, 0.3, 1);
                
                float2 vec = p[1].vertex.xy - p[0].vertex.xy;
                float2 offset = normalize(float2(-vec.y, vec.x)) * constScale * _Thickness * 0.5;
